...
class Photo_Controller extends Common_Controller {
  ...
  public function upload()
  {
    ...
    $file_validator->add_rules('photo', 
			       'upload::valid', 
			       'upload::required', 
	...
	array_push($tags, Photo_Model::$flickrtags[$photo_type]);
      ...
      if ($new_photo_id) {
	$f = flickr::makeFlickr();
	/* Using getSizes instead of getInfo for now... 
	   if we want title, descriptin, etc we'll need this.
	   $new_photo_info = $f->photos_getInfo($new_photo_id);
	   
	   if ($new_photo_info) {*/
	/*
	 Enough info to save at this point... let's try to find the best image size
	 and get actual width and height, otherwise we'll record them as 0 and go 
	 with Medium */
